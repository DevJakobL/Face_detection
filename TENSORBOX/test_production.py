...
def get_results(args, H, data_dir):
    ...
    with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=NUM_THREADS),
        ...
        for i in range(len(image_names)):
            ...
            if H['grayscale']:
                orig_img = imageio.imread('%s/%s' % (data_dir, image_name), mode = 'RGB' if random.random() < H['grayscale_prob'] else 'L')
                if len(orig_img.shape) < 3:
                    orig_img = cv2.cvtColor(orig_img, cv2.COLOR_GRAY2RGB)
            else:
                orig_img = imageio.imread('%s/%s' % (data_dir, image_name), mode = 'RGB')
            img = cv2.resize(orig_img, (H["image_height"], H["image_width"]))
            feed = {x_in: img}
            start_time = time()
